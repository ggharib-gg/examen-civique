...
A single-file, static French "examen civique" (naturalization civics test) trainer. The entire app — HTML, CSS, and JS — lives in `index.html` (~1200 lines); there is no build step, no bundler, no framework, and no package.json. `404.html` is a plain not-found page. Hosted on Vercel as a static site (no `vercel.json` — zero-config static hosting); there is no server-side code.
...
- **Screens** — four `<section>`s (`screen-start`, `screen-exam`, `screen-results`, `screen-flashcards`) toggled via the `screens` map / `showScreen(name)`, which just shows one and hides the rest. Flashcards render straight from `BANK` filtered by theme (`renderFlashcards`), independent of exam state.
